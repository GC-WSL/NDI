...
def im_detect_all(model, im, box_proposals=None, timers=None):
    ...
    if cfg.TEST.BBOX_AUG.ENABLED:
        ...
        scores, boxes, im_scale, blob_conv, box_feat = im_detect_bbox(
            model, im, cfg.TEST.SCALE, cfg.TEST.MAX_SIZE, box_proposals)
    timers['im_detect_bbox'].toc()

    #import ipdb
    #ipdb.set_trace()
    #_, _, cls_boxes_i = box_results_with_nms_and_limit(scores, boxes)
    ...
    return {'scores': scores, 'boxes' : boxes}#, 'box_feat':box_feat}#, blob_conv
...
def im_detect_bbox(model, im, target_scale, target_max_size, boxes=None):
    ...
    return scores, pred_boxes, im_scale, return_dict['blob_conv']#, return_dict['box_feat']
...
def box_results_with_nms_and_limit(scores, boxes):  # NOTE: support single-batch
    ...
    for j in range(1, num_classes):
        ...
        #dets_overlaps = bbox_overlaps(nms_dets, nms_dets)
