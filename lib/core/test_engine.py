...
def compute_colors_for_labels(labels):
    ...
    #colors = (colors % 255).astype("bool")
    return colors
def vis_det_bbox(img_path, cls_boxes_i, CATEGORIES):
    image = cv2.imread(img_path)
    template = "{}: {:.2f}"
    for label in range(len(cls_boxes_i)):
        boxes = cls_boxes_i[label]
        if boxes==[]:
            continue
        for box_id in range(boxes.shape[0]):
            ...
            x, y = bbox[:2]
            s = template.format(CATEGORIES[label], score)
            top_left, bottom_right = bbox[:2].tolist(), bbox[2:].tolist()
            image = cv2.rectangle(image, tuple(top_left), tuple(bottom_right), (0,255,0), 2)
            cv2.putText(
...
def test_net_on_dataset(
    ...
    for i, entry in enumerate(roidb):
        ...
        output_path = '{}/VIS'.format(output_dir)
        if not os.path.exists(output_path):
            os.makedirs(output_path)
        outimg_path = os.path.join(output_path, img_path.split('/')[-1])
        vis_im = vis_det_bbox(img_path, cls_boxes_i, CATEGORIES)
        cv2.imwrite(outimg_path, vis_im)
