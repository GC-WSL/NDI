...
from roi_data.augmentations import SSDAugmentation
...
class RoiDataLoader(data.Dataset):
    def __init__(self, roidb, num_classes, training=True):
        ...
        self._ssdaugmentation = SSDAugmentation()

    def __getitem__(self, index_tuple):
        index, ratio = index_tuple
        single_db = [self._roidb[index]]
        blobs, valid = get_minibatch(single_db, self._num_classes, self._ssdaugmentation)
